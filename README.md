...
2. 【高血压预警】，相信你在看我代码的时候会认识到这条预警的意义（~~被自己蠢哭~~）
3. 虽然周二在群里询问过题目有关问题，但是不保证提交的内容完全理解并实现了题目的要求（XD）。对于一个游戏服务器而言，设计还有相当多不完善的地方。请以程序实际效果为准。  
4. 在码代码的时候突然发现自己的思路就是让程序在几个状态之间来回横跳，于是干脆在main函数里用switch语句做了个简易版有限状态机...  
5. 另外，在Visual Studio Code的终端中运行时偶尔会出现客户端接收不到数据包的情况，但直接运行可执行程序时未出现上述问题。如果程序在意料之外的地方“无响应”，请尝试重新运行。（再注：这个bug疑似已经被定位到了原因，但是没时间再去改了~~（悲）~~，但是请记得，慎用<kbd>Enter</kbd>键）
...
* ### 客户端  
    ...
    5. 为简化设计，可用账户只有一个。用户名：<kbd>Admin</kbd>密码：<kbd>acltql</kbd>（~~注意区分大小写~~）。（再注：加上MD5之后对大小写不敏感了XD，所以不用区分大小写了）
* ### 服务端
...
---
（本部分内容较多，导览如下）
> [主要结构（struct）与常量](#主要结构struct与常量)  
> [客户端主要函数及功能介绍](#客户端主要函数及功能介绍)  
> [服务端主要函数及功能介绍](#服务端主要函数及功能介绍)  
> [通信数据包格式](#通信数据包格式)  
> [运行流程简介](#运行流程简介)   
* ### 主要结构（struct）与常量
```go
